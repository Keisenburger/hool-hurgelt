...
const Sidebar = ({
  ...
  // useEffect(() => {
  //   const fullPrice = cartItems.map((item) => {
  //     return;
  //   });
  //   setPrice(fullPrice);
  // }, [cartItems]);
  return (
    <div className="fixed top-0 right-0 w-2/6 h-full bg-[#303030] rounded-l-xl z-50 gap-6 flex flex-col p-8 shadow-2xl">
      ...
      <div className="w-full max-h-1/4 h-full bg-white rounded-[20px] p-4 flex flex-col gap-5">
        <p className="text-xl font-semibold text-[#71717A]">Payment info</p>
        <div className="flex flex-col gap-2 text-base">
          <div className="w-full flex justify-between">
            <p className="text-[#71717A]">Items </p>
            <p className="font-bold text-[#09090B] ">$</p>
          </div>
          <div className="w-full flex justify-between">
            <p className="text-[#71717A]">Shipping </p>
            <p className="font-bold text-[#09090B] ">0.99$</p>
          </div>
        </div>
        <div className="w-full border border-dashed border-[#09090b]"></div>
        <div className="w-full flex justify-between">
          <p className="text-[#71717A]">Total</p>
          <p className="font-bold text-[#09090B] ">$26.97</p>
        </div>
        <button className="px-8 py-2 bg-red-500 rounded-full text-[#FAFAFA] cursor-pointer">
          Checkout
        </button>
      </div>
    </div>
